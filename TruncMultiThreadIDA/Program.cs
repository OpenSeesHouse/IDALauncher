...
namespace IDALauncher
{
    static class Program
    {
        ...
        static void Main(string[] tmpFile)
        {
            ...
            try
            {
                numThreads = Convert.ToInt16(argv[4], System.Globalization.CultureInfo.InvariantCulture);
            }
            catch
            {
                MessageBox.Show($"Unacceptable numThreads: {argv[4]}", "Error", MessageBoxButtons.OK);
                return;

            }

            var theForm = new TruncMTForm(saStart, saStep, saStepIncr, recordList, numThreads, argv[5], argv[6]);
